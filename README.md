...
| 3 · security | `skill-scanner` (uv, 0.3.3) · `SkillSpector` (uv, v2.11.2) | fan out, findings merged | SARIF and policy checks from one, taint and dependency analysis from the other |
...
| SkillSpector | [NVIDIA/SkillSpector](https://github.com/NVIDIA/SkillSpector) | security | `uv tool install` from git, `v2.11.2` | NVIDIA's scanner: taint tracking, dependency checks, signatures. LLM analysis by default; `--no-llm` needs no credential. The only tool with a baseline file, so the only one `s` can suppress into |
