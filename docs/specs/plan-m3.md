...
- 2026-08-02 — **Local CLI installation via `pnpm install:cli`.** SkillGantry shipped as an npm package with a `bin` entry, but nothing put that binary on a PATH — the only way to run it was `node dist/cli/index.js` from the working tree. Created `scripts/install-cli.sh`: preflight (Node ≥ 24, pnpm, npm on PATH), `pnpm build`, `pnpm pack` into a staging dir, wipe-and-install into `~/.skillgantry/cli/` via `npm install --prefix`, symlink `~/.local/bin/skillgantry`, verify by invocation. Both paths overridable via `SG_HOME` and `SG_BIN_DIR`, which is how the acceptance test installs without touching a real home. Added `"install:cli": "scripts/install-cli.sh"` to `package.json`. This is a deliberate exception to R3.1 ("never into the user's global environment"), which governs managed tools, not SkillGantry's own binary. Created `tests/acceptance/install-cli.test.ts` asserting semver output, link resolution, re-runnability, and that the real `~/.local/bin` is untouched. Updated design §2 (distribution paragraph) and §16 (test-strategy table), and CLAUDE.md's commands block. No requirement change: R13.5 already owns npm distribution. Original plan recoverable via git history as `plan_m3-install-as-terminal-command.md`.
- 2026-08-02 — **The `credentials-and-repo` step did not work.** M3 shipped it with no test; the wizard suite stopped at `install-and-verify`. All five defects below were reproduced before being fixed. Design §5.3 gains the `inspectRepo` contract and the skip rule. In order of how badly each broke the step:
  1. **The typed path was never displayed.** `Setup.tsx` rendered `state.repoPath`, which is only set *after* a successful registration, and the in-progress buffer lived in `setup-app.tsx` and was never passed down. Typing produced a byte-identical frame, so the wizard read as frozen. `draftPath` is now a prop, with an inverse-block cursor so an empty field still looks focused.
  1. **A pasted path was dropped whole.** The handler guarded on `input.length === 1`, and a terminal paste arrives as one multi-character chunk. Ink 7's `usePaste` now takes it on its own channel; the guard is `input.length > 0`.
  1. **A rejected registration killed the wizard.** `void driver.registerRepo(path).then(…)` had no rejection handler, so `already registered: <path>` reached Node's default handler and terminated the process with the terminal in raw mode. Registration failure is now a message on the step the user is still standing on.
  1. **`~` was not expanded.** `resolve()` treats it as an ordinary segment, so `~/dev/skills` registered `<cwd>/~/dev/skills` — a repo that could never hold a skill, reported as success. Fixed in `canonicalisePath`, so the headless path gets it too, and `registerRepo` now refuses a path that is not a directory rather than letting discovery throw from inside `readdir`.
  1. **Enter on an empty path was a silent no-op**, as was Enter on any step whose `canEnter` precondition was unmet. Both now say which precondition is unsatisfied.
Beyond the defects: `inspectRepo` (new, `src/core/config/config.ts`, on `SetupDriver`) resolves the path as it is typed and reports the canonical path, whether it is a directory, whether it is already registered, and its skill count. It is debounced 120 ms and sequence-guarded, so a slow stat on a prefix cannot overwrite the verdict for the full path. An empty directory warns and still registers. `done` is now reachable by Ctrl+D with no repo (`repoSkipped` on `SetupState`); requiring one left a user setting up before their skills repo existed with no exit but Ctrl+C.
- 2026-08-02 — Styling: a step counter and a five-step progress rail replace the internal state enum the title had been printing (`Credentials and repo (credentials-and-repo)`), and the frame is capped at 84 columns instead of stretching across the terminal. Nine new cases in `tests/tui/setup-wizard.test.tsx` cover the whole step, including bracketed paste and Ctrl+D; `tests/core/config.test.ts` covers `~` expansion and `inspectRepo`. The Ink 6.8 → 7.1 bump this depends on is recorded in [plan-m2.md](plan-m2.md).