...
- 2026-08-02 — **The `credentials-and-repo` step did not work.** M3 shipped it with no test; the wizard suite stopped at `install-and-verify`. All five defects below were reproduced before being fixed. Design §5.3 gains the `inspectRepo` contract and the skip rule. In order of how badly each broke the step:
  ...
  - **The typed path was never displayed.** `Setup.tsx` rendered `state.repoPath`, which is only set *after* a successful registration, and the in-progress buffer lived in `setup-app.tsx` and was never passed down. Typing produced a byte-identical frame, so the wizard read as frozen. `draftPath` is now a prop, with an inverse-block cursor so an empty field still looks focused.
  - **A pasted path was dropped whole.** The handler guarded on `input.length === 1`, and a terminal paste arrives as one multi-character chunk. Ink 7's `usePaste` now takes it on its own channel; the guard is `input.length > 0`.
  - **A rejected registration killed the wizard.** `void driver.registerRepo(path).then(…)` had no rejection handler, so `already registered: <path>` reached Node's default handler and terminated the process with the terminal in raw mode. Registration failure is now a message on the step the user is still standing on.
  - **`~` was not expanded.** `resolve()` treats it as an ordinary segment, so `~/dev/skills` registered `<cwd>/~/dev/skills` — a repo that could never hold a skill, reported as success. Fixed in `canonicalisePath`, so the headless path gets it too, and `registerRepo` now refuses a path that is not a directory rather than letting discovery throw from inside `readdir`.
  - **Enter on an empty path was a silent no-op**, as was Enter on any step whose `canEnter` precondition was unmet. Both now say which precondition is unsatisfied.
