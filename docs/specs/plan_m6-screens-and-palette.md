...
- 2026-08-05 — **A polish pass over the screens this milestone shipped.** No new capability; four defects and a vocabulary that had drifted while six screens were built one at a time. The Work screen's half of the same pass is in [plan_m2-queue-and-tui.md](plan_m2-queue-and-tui.md)'s changelog for this date.
  ...
  - **Five phrasings of "you are seeing part of a list".** Unified via `overflowNotice()`, which states the position first and takes the recovery text from the caller.
  - **Help hid its own keys.** Regrouped by scope (global first), merged co-travelling rows. Seventeen rows; fits 80×24.
  - **Copy.** Pluralisation fixes; missing `q quit` in the Issues footer.
- 2026-08-12 — **the credential rows now answer the gate's question.** `credentialsOf` in `src/cli/gantry-views.ts` tested a tool's requirement against `loadEnvFile().vars` while the gate at `stages/adapter-stage.ts` tested it against `ctx.env`, which is `spawnEnv(process.env, env.vars)` — so a key the user's shell exported read `missing` on the one screen written to answer whether a tool can run, and satisfied the run seconds later. Rev 28's derived `ANTHROPIC_API_KEY` widened the same gap: a gateway `.env` carrying only the auth token diverged for any tool requiring the API-key form. The gate is the correct side, since a child process really does inherit the ambient environment and skipping a tool over a key it would have found is a false skip. Fixed by evaluating the row against the same expression `tui-command.ts` hands the pipeline, through one predicate: `satisfyingAlternatives` in `adapters/types.ts`, which `credentialsSatisfied` is now defined in terms of. Two hand-rolled copies of that `.every` test are what let the sides drift at all. A row satisfied by a key `.env` does not hold reads `via <provider> (shell)`, because the group's heading names the file and R11.7 asks a value's source to be beside it; presence only, so R7.3 is untouched. R11.7 amended in place as revision 29. §4.1 corrected, having claimed that Settings, provenance *and* the gate all reported the file when the gate never has, and §14.2 gained the row's rule.
  **`skillhoneSettings(env.vars)` stays file-only, deliberately.** It composes R3.10's configuration file rather than reading one, and sourcing it from the ambient environment would write a shell's secrets into a document SkillGantry owns.
  **Known gap: provenance keeps the blind spot.** `provenanceOf(env.vars)` still reads the file alone, so a gateway configured only in the shell is recorded as no provider on every run. Left as-is rather than fixed in passing: it changes what a recorded provenance means, and is worth its own decision.
