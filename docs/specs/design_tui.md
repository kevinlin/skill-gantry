...
**A staging surface holds the document before it accepts an edit** (R11.8, rev 31). Every staging action resolves `state.staged ?? state.settings?.config`, and that view was loaded on one condition: the Settings screen being up. The wizard is a screen the palette reaches from anywhere, so `:setup` from Work mounted it against no document — `stage-repo`, `stage-repo-path` and `stage-selection` each returned the state untouched, the wizard walked on to `done`, and a user registering their second repo was told it was registered while nothing had been staged, written or said. The screen therefore loads the same view Settings does, and holds the wizard behind a `reading the configuration…` row until it arrives, in a component the config is a required prop of. A conditional around the hook is not available — hook order is fixed — and mounting the wizard early would break the seed as well as the staging: `initialSetupState` is a lazy reducer init that runs once, so a wizard mounted before the read landed renders a configured machine as having no tools selected, which is the thing this section seeded it to prevent.

**The done line belongs to the caller, because the two callers did different things.** `skillgantry setup` wrote the file and says `Registered <path>.`; this screen has staged and says so. One sentence for both read as a completed registration one keystroke before the change set existed — the point at which `q` would discard it.

**A key the footer names is a key that screen handles.** The staged done screen footers `c apply the change set`, and `c` acts from there: it goes to Settings and opens the change set, the one keystroke between staging and the write. Worded as an instruction about another screen — `q settings · c there applies the change set` — it was pressed where it was read, did nothing, and the user reported that the repo could not be saved. Same defect as the Issues footer that once omitted `q quit` while `q` quit from there, inverted: a key named where it does not act. It is offered only by the caller that has something to apply, which is why the hook takes `onConfirm` and `skillgantry setup` leaves it undefined.

**The apply says what a repo change does not do.** The skill list is discovered once, by `startTui`, before the session exists — so a repo this write adds is on Settings and absent from the list the user works in. The pane says "next launch" before the write; the flash says it again after, because the place the user then looks for the repo is the screen that cannot show it yet.

**`q` refuses while a change set is held.** The refusal guards the staged document rather than the Settings screen, because the wizard stages from a screen that is not Settings, and that is the case with the most to lose; it names where the change set is (`:settings, then c applies`) on R11.9's footer row. That row is why every screen now renders `state.flash`: Work was the only one that did, so a refusal raised anywhere else was invisible, which is worse than the key doing nothing at all.
...
**The Overview card is sized by rows, not by columns** (R11.12). `layoutFor` gains `overview: 'full' | 'compact' | 'none'` and picks the largest tier leaving `SKILL_LIST_MIN` rows in the list. `full` is six rows — one bar per gate stage, the issue summary, the slowest stage's median, and the dashboard link. `compact` is four: the bars and that link. Rows and not a width band because the card competes for the left column's *height*: a 200×20 terminal has cells to spare and nothing to give. `standard` only; `narrow` stacks the list above the rail and has no column for a card.

The dashboard link sits on both tiers, because a key advertised on the largest tier alone is a key most terminal heights never show. It reads `[0] full dashboard →` rather than `0  full dashboard →`, since the rows above it open with counts and a bare digit in that column is read as one more. The row costs `OVERVIEW_ROWS.compact` one and moves exactly one boundary — at 21 rows the card drops to `none`. The panel's `hint` was the free alternative and does not work: `Panel` truncates the title first and gives the hint whatever is left, so at the 22-cell column the whole 76–109 band uses, `0 dashboard` renders as `0 dashb…`.

Two assertions hold the arithmetic. `OVERVIEW_ROWS[tier]` and the row count `overviewRows` emits are asserted equal — until this change they agreed only by coincidence, `GATE_STAGES` being three long, and a tier allocated a row its builder never fills fails silently in a direction no frame assertion catches. And every tier boundary is asserted against `layoutFor` rather than at a named terminal size, because `overviewRows(stats, tier, width)` and `bar(pct, cells)` are pure and sit in `rows.ts` beside `dashboardRows`: a later change to what the chrome costs would otherwise move the boundary and break a test that was describing arithmetic rather than a rule.

Both the bar's cell count and the label's width derive from the width, and **the label shortens first** — `sec` still names the stage, while a two-cell bar shows no proportion at all. Reserving a constant instead cut off the percentage at a 22-cell column, which is the one number the bar exists to quantify. The card's data is one unfiltered `views.dashboard({})`, read at launch and on `:refresh`; `0` goes to the Dashboard screen rather than becoming a sixth entry in §14.2's precedence order.
...
**`SuppressPane`** is a sibling of `ReviewPane` and `ConfirmPane`. It renders diff text, so `ReviewPane`'s diff body is extracted into a shared `DiffBody` used by both — §14.6's one-frame-one-renderer rule, and these are the panes whose `a` writes the user's repo, so this is exactly where a colour has to mean what it means everywhere else.
...
**The `r` handler grows one branch beside release's.** `wanted.includes('release')` opens `ReleaseTargetPane`; `wanted.includes('optimise')` opens `OptimisePane`. Both carry §14.9's mixed-mark refusal and its mark-clearing rule unchanged, for §14.9's stated reasons.
...
**It enqueues nothing and applies nothing.** `beginOptimise` calls `views.planOptimise(skillId)`, a pre-commitment read on §14.9's `planRelease` pattern. The port returns the finished R6.12 body rather than its ingredients, so the pane renders and decides nothing. `y` copies through the existing `osc52.ts` and reports the unavailable case as well as the copy, since an action able to report only success is the failure §14.3 exists to prevent; `j`/`k` scroll; `esc` closes. There is no `a`, because there is nothing to apply.

**Precedence** slots after `ConfirmPane` and before the setup screen: §14.2 orders the modals by what a keystroke can destroy, and this pane builds no job and writes no byte.
...
**`planEvals` on the views port**, returning the finished body beside `hasSuite` and the missing-dependency list — the fourth read on §14.9's `planRelease` pattern. Its rejection — skill-up unlocked, or skill-upper reachable in no runtime directory — reaches the same guard-then-flash shape `y`, `o` and `s` use, naming the tool and `skillgantry setup`.
