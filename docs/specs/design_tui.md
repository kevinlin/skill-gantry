...
**The done line belongs to the caller, because the two callers did different things.** `skillgantry setup` wrote the file and says `Registered <path>.`; this screen has staged and says so, naming the keystroke that applies it. One sentence for both read as a completed registration one keystroke before the change set existed — the point at which `q` would discard it.
