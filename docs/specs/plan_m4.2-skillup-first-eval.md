...
- 2026-08-12 — **the gateway credential now reaches the spawn.** Every evaluate run against a configured gateway logged `ANTHROPIC_API_KEY not set, claude-code will rely on existing login state if available`, once per case. The cause was not skill-up's: `~/.skillgantry/.env` holds the pair `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN`, all three spawn sites handed the child the literal merge `{ ...process.env, ...env.vars }`, and skill-up probes `ANTHROPIC_API_KEY` alone for the `claude_code` engine. The run passed anyway — the `claude` CLI reads the auth token itself — but a tool that gated rather than warned would have skipped against a credential that was there all along, and M4.1's `profileEnv` had already established that a gateway token needs both forms, in the file it composes rather than at the spawn R7.3 is written about. Fixed by one pure `spawnEnv(ambient, vars)` in `config/env.ts`, used by `run-command.ts`, `tui-command.ts` and `release-command.ts`, deriving `ANTHROPIC_API_KEY` from the token only when a base URL is configured, never overwriting an explicit key, and never against `api.anthropic.com`, where the two forms are different credentials. `loadEnvFile` still returns the file's literal keys, so Settings and provenance report what the file holds. Redaction was already covered: the derived value is the token value, which the `_TOKEN` suffix put in the scrub set. R7.3 amended in place as revision 28, with §4.1 gaining the child-environment composition and claiming it.
  This plan's Task 6 prompt named `ANTHROPIC_API_KEY` while the setup wizard named `ANTHROPIC_AUTH_TOKEN` — one credential, two surfaces, different keys — so `CREDENTIAL_KEYS` gained the gateway pair as a named alternative and §9.4b says so. Keys only; R6.13 holds and the builder still takes no credential argument.
  **skill-up's gate is unchanged, deliberately.** It keeps `credentials: { kind: 'none' }`: it delegates auth to the engine CLI declared in the skill's own `evals/eval.yaml`, so a `one-of` requirement would skip a user logged into `claude` with no key set, and R4.2b's clean-environment smoke test requires that invocation to succeed. The known gap recorded above — skill-up cannot be `skipped` for want of credentials — stands.
