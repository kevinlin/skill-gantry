...
*Superseded in part:* four of these eight are not shipping. M3 found agentskills, SkillOpt and SkillHone unpublished in installable form (probe output in [plan_m3-tools-module.md](plan_m3-tools-module.md)); promptfoo is removed by §10 below. The catalogue is the record of what exists, not this paragraph.
...
## 13. SkillHone reinstated as a skill bundle

Supersedes §10's SkillHone omission and D7's "both optimise candidates unpublished". Both were verdicts about *CLI publication*, and they were correct as such: `Tencent/SkillHone` publishes no package on any index, no tags, and no executable that answers a version argv. What the probe did not ask was whether the tool is published at all. It is — as six agent skills under `skills/`, each a `SKILL.md` with its scripts, references and agents. Probed 2026-08-09 at `7d565839fb4dc74f9c77f09ace660e1c0484e048`.

The cost of the omission was not abstract. Optimise had no member in any preset, its rail column was permanently dead, and R11.20 was written to refuse a mark on it — a requirement whose stated reason was that the stage had nothing behind it.

**D28. A catalogued tool need not have an executable.**
`git-skill`: clone at a pinned sha under the tool root, one symlink per bundled skill directory into each detected runtime skills directory, dependencies in a venv the managed uv builds.
*Why:* every field the three existing kinds need — a package spec, a registry, a version argv — is absent here, and every one it does need is absent from them. A fourth kind is smaller than teaching the three to be optional. `stage: null` keeps it out of `stageTools`, vercel `skills` being the precedent for a tool the catalogue installs and no stage selects; an id the adapter registry does not hold fails the whole run at `plan()`, so this is load-bearing rather than tidy.
*Why per-skill symlinks and not `cp -r`:* upstream advises the copy, and the reason it gives is that other skills already live in the target directory and a symlinked parent would clash with them. That is an argument against linking the parent `skills/`, not against linking a member. Per-skill links keep one clone as the single copy, so a pin change is a `git checkout` rather than six directory replacements.
*Why a venv:* upstream's documented step runs `pip install` against the user's global interpreter — `litellm[proxy]`, GitPython, the Anthropic SDK. R3.1 forbids that, and upstream explicitly blesses the alternative, requiring only that the absolute interpreter path be used for SkillHone's scripts.
*Rejected:* omitting the tool a second time (the reason had expired); vendoring the bundle into SkillGantry's own package (a fork with no update path); installing globally as documented (R3.1).

**D29. The optimise stage acquires an action, not a run.**
