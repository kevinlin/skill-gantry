...
| [plan_m3-setup-repo-edit.md](plan_m3-setup-repo-edit.md) | M3 (extension) | In progress | The setup repo step shows and edits what is registered (R3.12): the id-preserving `withRepoPath`, `updateRepo` on the driver, a cursor over the registered list, and a verdict that tells this repo's path from another's |
