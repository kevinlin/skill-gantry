...
Each requirement carries the decision it derives from and, where the check is not self-evident, how to verify it. `MUST` is binding for v1; `MUST NOT` marks an explicit exclusion. Requirements amended in revision 2 are marked *(rev 2)* with the review finding that prompted the change.
...
- **R2.8** Every run MUST record a `skillDigest` computed over the skill's file tree, excluding the workspace directory, `.git/` and any snapshot directory. For a git repo the run MUST additionally record the HEAD commit and whether the skill path is dirty. *(rev 2, finding 8)*
  *Verify:* editing any tracked byte of the skill changes the digest; writing a new run artefact into the workspace does not.
...
- **R3.3** The lockfile MUST record, per tool, the requested pin, the resolved version, the **resolved absolute executable path**, the install kind, and install and verification timestamps. *(rev 2, finding 10)*
...
- **R4.2** The manifest MUST declare id, stage, policy, mutating flag, detectable rule classes, credential requirement, install spec including the binary name, invocation argv, version argv, expected artefacts, and default timeout. (Adapter contract decision)
...
- **R5.11** Tool and stage outcomes MUST each be a closed set, and the reduction from tool outcomes to a stage outcome MUST be total over every non-empty combination. *(rev 2, finding 4)*
  *Verify:* a test enumerates the full Cartesian product of tool outcomes and asserts exactly one stage outcome for each.
...
- **R6.4** SkillGantry MUST maintain a `latest` pointer and an append-only `runs/index.ndjson` per skill workspace. *(rev 2, finding 9)*
  *Rationale:* a single rewritten JSON document can be truncated mid-write; one object per line truncates at a line boundary instead.
...
- **R6.7** Concurrent runs against one skill MUST NOT lose an index entry, leave `latest` nondeterministic, or collide on a run directory. Run identifiers MUST be claimed by exclusive directory creation rather than assumed unique. *(rev 2, finding 9)*
  *Verify:* two runs finalising the same skill simultaneously both appear in the index and `latest` names the later one.
...
- **R8.8** An issue MUST be closed automatically only when the tool that most recently detected it completed with outcome `passed` or `failed` in a later run and did not report it. (Failure-policy decision)
  *Verify:* a run in which the security tool errors closes zero issues; likewise for a skipped tool.
...
- **R8.12** A tool's reconciliation scope MUST include its own unmapped rule classes in addition to its declared detectable classes. *(rev 2, finding 6)*
  *Rationale:* unmapped classes are outside any tool's declared set, so they could otherwise never close.
...
- **R9.4** Release MUST produce an archive at `<repoRoot>/<skillName>_<version>.zip`. *(rev 2, finding 8)*
- **R9.5** Release MUST write an evidence bundle into the sidecar containing the validate result, eval report, merged security findings, the tool lockfile, the skill digest and the manifest mode. *(rev 2, finding 8)*
...
- **R10.3** For a git-backed repo, SkillGantry MUST refuse to run a mutating stage against a skill directory with uncommitted changes unless the user overrides. (D17)
  *Rationale:* the worktree starts at HEAD, so uncommitted work would be absent from the tool's input and lost from the resulting diff.
- **R10.4** For a repo not under git, the sandbox MUST copy every declared scope path to `snapshot-pre/` within the run directory before any write, preserving file modes, and excluding the workspace directory and `.git/`. *(rev 2, findings 1, 7)*
...
- **R12.5** `doctor` and `release` MUST be available as headless subcommands. *(rev 2, finding 11)*
...
- **R13.6** Every P1 finding in the design review MUST have a corresponding contract test. *(rev 2, finding 12)*

---

## Milestone ownership

Derived from D19 and the traceability matrix in [design.md §17](design.md). Exactly one milestone owns each requirement.

| Milestone | Requirements owned | Exit criteria |
|---|---|---|
| M1 | R1.1–R1.3, R1.5, R2, R3.3 (schema only), R4.1–R4.5, R4.9–R4.12, R5.1, R5.9, R5.11, R6.1–R6.6, R6.8, R7, R8.1–R8.8, R8.10–R8.14, R12.1–R12.3, R13.1–R13.6 | `skillgantry run <skill> --stage security --json` writes a complete run directory and populates the ledger; a whitespace-only edit changes no fingerprint; an errored tool closes no issue; no secret appears in any log; a hanging grandchild process is killed on timeout; the packed npm artefact runs from a clean prefix |
| M2 | R5.3–R5.8, R5.10, R5.12–R5.14, R6.7, R11.1–R11.2, R11.4–R11.6 | Work screen renders live state from the M1 engine with no engine change; two concurrent runs on one skill finalise without loss; cancellation works in all four phases |
| M3 | R3.1–R3.2, R3.4–R3.9 | A clean machine reaches a verified toolchain through the wizard alone; doctor reports all four drift kinds |
| M4 | R4.6–R4.8 | Fan-out merges findings from both scanners into single issues with two detections; colliding filenames both survive |
| M5 | R1.4, R5.2, R9, R10, R12.4–R12.5 | Both sandbox strategies pass apply, rollback and crash-recovery tests over all five change kinds; the dirty-skill guard holds; digest mismatch blocks release; the no-manifest path releases correctly |
