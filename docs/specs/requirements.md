...
- **R3.5a** SkillGantry MUST additionally install vercel `skills`, which the release stage invokes for the installability check. The total installed is therefore R3.5's catalogue plus one, which as the catalogue stands is five. *(rev 13: the figure read "nine", which counted revision 2's eight adapter-backed tools; M3 dropped four of those after probing — [plan-m3.md](plan-m3.md), [decision-log.md](decision-log.md) §10 — and design.md §3 has said five since. Stated as a rule over the catalogue rather than a count, for R3.5b's reason: a requirement stated as a count goes wrong every time the catalogue moves.)* (D9, R9.6)
