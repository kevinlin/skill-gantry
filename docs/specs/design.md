...
**The Log pane is not replayed.** Its lines live in one session-wide ring buffer shared by every skill, while everything above is per-skill state, so seeding it has ambiguous semantics the moment one skill is running and another is selected. Instead its empty case names the recorded run's directory, where the full log already is (R11.5) — one row, the row it already spends.
