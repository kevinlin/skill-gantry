...
Fourteen modules under `src/core/`. Rule applied throughout: a module that owns I/O does not also own decisions.

Release is a module, not an adapter: it has no external tool to wrap, so it has no manifest and no `parse`. It does depend on `tools`, because vercel `skills` must be installed for the installability check. Seven tools are installed in total: four adapter-backed, vercel `skills`, and the two `git-skill` bundles §5.1a describes. Revision 2 planned eight adapter-backed tools, one per D7 candidate; M3 dropped four of them after probing — agentskills and SkillOpt are published nowhere installable, promptfoo drives off a per-skill config no skill carries (decision-log §10), and SkillHone returned as a bundle rather than a CLI.
...
| `suppress` | §12.5's narrow write path: target resolution, entry substitution, document append, diff-confirm-recheck-rename | `adapters`, `discovery`, `isolation`, `ledger`, `tools` | fs |
| `upgrade` | §20's check and apply: `upgrade.json`, eligibility, release resolution, versioned install and relink | `release`, `tools` | fs, net, subprocess |
...
`upgrade` reaching `release` is the same shape: one import of the semver comparator, because a second comparator is how the release path and the upgrade path come to disagree about what "newer" means. It opens no ledger, which puts it with `tools`, `queue` and `isolation` rather than with `suppress`.
...
**The child environment of a spawned tool** is composed once, by `spawnEnv(process.env, vars)`, and the runner passes it through untouched — there is no allowlist. Three layers, in order: the ambient environment, then `.env`, then one derived key. `loadEnvFile` keeps returning the file's literal contents (R7.1), so provenance records what the file holds; the derivation belongs to the spawn, which is the thing R7.3 is about.

**The credential gate and Settings both read that composed environment**, through one predicate: `satisfyingAlternatives` in `adapters/types.ts`, which `credentialsSatisfied` is now defined in terms of. The gate has to, since a child process really does inherit the ambient environment and a tool skipped for want of a key its own shell exports is a false skip. Settings has to for the same fact seen from the other side: reading `.env` alone, the row said `missing` for a credential the run then accepted, under a heading naming a file that held nothing to fix. That is R11.7 rev 29, and it is why a row satisfied from outside the file says `(shell)` rather than passing the ambient value off as the file's. Provenance is the one reader that stays file-literal, a known gap rather than a decision: a gateway configured only in the shell is recorded as no provider at all. §5.4's `skillhoneSettings` stays file-only deliberately, because it *composes a file*, and sourcing it from the ambient environment would write a shell's secrets into a document SkillGantry owns.
...
Basename matching is otherwise deliberately gone. Revision 2 excluded "any `snapshot-pre/` directory", which would have let a legitimately named skill directory change without invalidating gate evidence. Snapshots live at `<run>/snapshot-pre/` inside the workspace, so the workspace exclusion already covers them. The two OS names are the exception because they are safe for the reason `snapshot-pre` was not: both are reserved by an operating system, so no legitimate skill file carries them (R2.9 rev 17).

**The sandbox must reproduce this manifest, not git's view of it.** The digest, the snapshot and the archive all read the manifest, which walks the filesystem. The git strategy is the one derivation that does not: it builds a worktree at HEAD and seeds it from `git status`, which hides ignored files by default. A candidate file the repo ignores was therefore in the digest the gates recorded and absent from the sandbox, so R9.9 compared two different sets and refused — and re-running the gates reproduced the same live digest and refused again, making the release structurally impossible rather than merely blocked (run `019fe590`). `dirtyPaths` passes `--ignored` so that class is seeded like any other, and membership is still asked of the manifest, so a path the candidate excludes is reported by git and dropped here. That ignored class is also where `.DS_Store` sat, which is why the row above matters twice: it keeps Finder's droppings out of the digest and out of the archive a consumer installs.
...
Doctor reports four drift kinds per tool, each meaning a tool cannot run: `missing` (in lock, absent on disk), `unverifiable` (present, will not run), `version-drift` (runs, reports a version other than `resolvedVersion`), and `unlocked` (installed under the tool root but absent from the lock).

Nine further conditions are reported and do **not** fail the report:

| Condition | What it names |
|---|---|
| `integrity-unverified` | a lock entry recording `integrity: "none"` per §5.2 |
| `lifecycle-drift` | the ledger's lifecycle cache disagreeing with frontmatter, per §13 |
| `rule-map-pending` | a ledger whose applied rule-map version trails the shipped one, per §10.6 |
| `skillhone-deps` | a managed venv whose interpreter cannot import the bundle's requirements |
| `claude-cli-missing` | no `claude` on PATH, which `claude-agent-sdk` shells out to — so its absence surfaces not at install but as a `FileNotFoundError` at the optimisation loop's first run |
| `skillhone-config-missing` / `-unmanaged` / `-stale` | §5.4's three states of the tool-owned settings file |
| `skill-link-unmanaged` | a runtime skills directory holding a bundled skill through a link SkillGantry did not create |
| `skillgantry-outdated` | a published release newer than the running build (§20), named beside `skillgantry upgrade` |

All of them are R3.7's probe-and-report rule — applied to a tool's runtime dependency, its own configuration, a directory it shares with the user, and SkillGantry's own binary rather than to a host runtime: named, never installed, never written. Failing a report on a machine that is fine is how a doctor report stops being read.

`skillgantry-outdated` is not a `ToolDriftKind`: SkillGantry is not one of the tools in the lock, and widening that union would put it into every per-tool loop over the kinds. Its check runs in `src/cli/` and is passed in as data, exactly as the lifecycle cache already is, so `src/core/tools/` gains no network dependency.

`skill-link-unmanaged` needs a kind of its own because the two states beside it already have one and neither describes it. A catalogued, selected tool with no lock entry is `unlocked`; a link SkillGantry made and something deleted fails `verifyGitSkill` into `missing`. The third is a foreign copy, which *works* — the agent has the skill, it is simply not ours — so its detail names the link, its target, and that removing it and re-running `skillgantry setup` puts the pinned copy in place. `rule-map-pending` is resolved by `skillgantry doctor --migrate-rule-map`, the explicit trigger R8.14 requires — the migration never runs as a side effect of opening the ledger.

One condition is about a skill rather than a tool: `frontmatter-unreadable`, a `SKILL.md` whose frontmatter block is present and would not parse (R2.5). Reported, never failing the report, for the nine above's reason. It is a `SkillFinding` on its own array rather than a `ToolDriftKind`, for `skillgantry-outdated`'s reason: a skill is not one of the tools in the lock. It reads no file — `SkillRef.frontmatterReadable` is what discovery saw when it parsed, so doctor filters the skills it was already given, while `lifecycleDrift` re-reads because comparing the file against the ledger's cache is a different question.
...
The body names the skill directory, the repo root, the declared version, the commit and dirty flag and the skill digest; the newest recorded run's per-stage outcomes and the absolute path of each tool's report; the eval assets under `<skill>/evals/`; the managed interpreter, the SkillHone location and its sha; the handoff to the top-level `skillhone` skill, which dispatches to its own sub-skills; and upstream's own notice that some of its workflows use bypass mode and local subprocess execution. Absent evidence is stated rather than omitted — a section that vanishes reads as a builder that failed.

Its install argument is plain fields rather than a type imported from `tools`, so the builder adds no §3 edge — the property §9.4 records as the reason `fix-prompt.ts` lives in `stages` at all. `src/cli/gantry-views.ts` reads the lock and flattens it, which is where the ledger and the process table are already reachable.
...
**What is omitted and what is kept, with a reason each, is R10.12's** — every piece of §12's machinery this path drops answers a problem it does not have, and every piece it keeps guards one it does. Two clauses R10.12 leaves to the design: the atomic rename is kept because a half-written baseline is one the tool exits 2 on, and the abort covers absent-became-present as well as drift, since a preimage of `null` that finds a file at recheck means someone created the baseline while the diff sat on screen.
...
Specified in [design_tui.md](design_tui.md), which holds §14 through §14.14 under their own numbers: the store and render discipline, the responsive row budget, Settings, the fix-prompt copy, the queue's progress reporting, run rehydration, the Work screen overhaul, accepting a finding, the full-length detail view, the release target, the optimise and eval-bootstrap prompt surfaces, the two-level repo and skill list, the setup repo step, and the upgrade prompt. It is a separate file because it is a fifth of this document and no engine change reads it.
...
| R3.12 the setup repo step | 5.3, [design_tui.md](design_tui.md) 14.13 |
...
| R11 terminal interface | [design_tui.md](design_tui.md) 14, 14.1, 14.2, 14.3, 14.5, 14.6, 14.7, 14.8, 14.9, 14.10, 14.11, 14.12 |
...
Every pass below is recorded in full somewhere else — the two reviews are their own documents, and each milestone's deviations are the last section of its plan. What changed is in the sections above. This table is an index: what a pass was about, which sections it touched, and where its reasoning lives. Passes are listed in the order they landed.

| Pass | What it was about | Sections | Record |
|---|---|---|---|
| Revision 2 | The first review's twelve findings | §4.3, §4.4, §6, §7, §8, §9, §9.1–§9.3, §10.3–§10.5, §11.1, §11.4, §12.1, §12.4, §16, and the coverage gaps that became §4, §5, §13, §17 | [design-review-r1.md](design-review-r1.md) |
| Revision 3 | The second review's eleven findings | §4.4, §4.5, §5.1, §5.2, §7, §8.1, §9.1, §9.2, §10.1, §10.4, §12.2, §12.4, §13, §17 | [design-review-r2.md](design-review-r2.md) |
| M3 planning | The catalogue as install authority and the registry as run authority | §5.1a, §5.2, §5.3 | [plan_m3-tools-module.md](plan_m3-tools-module.md) |
| M5 | Six amendments taken from building the mutation and release paths | §3, §8.1, §12.1–§12.4, §15 | [plan_m5-mutation-and-release.md](plan_m5-mutation-and-release.md) |
| M6 | A coding-agent prompt as the deliverable for a stage that found something, then a tool's own suppression file honoured from argv to Issues screen | §7, §8.1, §9.4, §10.1, §10.4–§10.7, §12.4, §14, §14.3, §15 | [plan_m6.2](plan_m6.2-fix-prompts-for-stage-findings.md), [plan_m6.3](plan_m6.3-respect-skillspector-baseline.md) |
| M7 | The Work screen overhaul, and the corrections measuring a rendered frame forced | §14, §14.1, §14.3, §14.6 | [plan_m7-work-screen-overhaul.md](plan_m7-work-screen-overhaul.md) |
| M7 extension | Key routing, and the full-length surface a truncating pane cannot be | §14.2, §14.6, §14.8 | [plan_m7.1-work-screen-navigation.md](plan_m7.1-work-screen-navigation.md) |
| M8 | Writing the file M6 taught SkillGantry to read | §4.4, §7, §12.5, §14.7, §15 | [plan_m8-suppress-finding.md](plan_m8-suppress-finding.md) |
| M4.1 | SkillHone catalogued as a skill bundle rather than a CLI, giving optimise something behind it; then the configuration file install left uncomposed | §5.1, §5.2, §5.3, §5.4, §9.4a, §14.10, §15 | [plan_m4.1-skillhone-optimise.md](plan_m4.1-skillhone-optimise.md) |
| M4.2 | A way for the evaluate gate to start | §5.1a, §5.2, §5.3, §9.4b, §14.11, §15 | [plan_m4.2-skillup-first-eval.md](plan_m4.2-skillup-first-eval.md) |
| M9 | Distribution as something the product does rather than the maintainer | §5.3, §14.14, §15, §20 | [plan_m9-version-check-and-upgrade.md](plan_m9-version-check-and-upgrade.md) |
| M1.1 | A run directory named for when it started, separated from the identity everything joins on; then the screen, which still named a run by its id | §9, §9.1, §9.2, §10.3, §11.3, §13, §14.8, §15 | [plan_m1.1-timestamped-run-directories.md](plan_m1.1-timestamped-run-directories.md) |
| M7.2 | Two levels in the list column; then the row label that undid it, and the doctor condition that says why a skill has no name | §5.3, §14.12 | [plan_m7.2-repo-skill-navigation.md](plan_m7.2-repo-skill-navigation.md) |
