...
- 2026-08-13 — **Fixed: the done screen named a key it did not handle.** The fix below footered `q settings · c there applies the change set`, and `c` was bound to nothing on that screen — so it was pressed there, nothing happened, and the repo read as unsaveable a second time. `c` now acts from the done screen: `useSetupSession` takes an `onConfirm` the screen wires to Settings plus the change set, and `skillgantry setup` leaves it undefined, so the key is advertised exactly where it works. The apply also flashes that a registered repo's skills are listed after a relaunch — the list is discovered once by `startTui`, so Settings shows the repo and the Work screen cannot until the session restarts.
- 2026-08-13 — **Fixed: the screen's wizard staged nothing unless it was opened from Settings.** Task 16's screen stages into `state.staged ?? state.settings?.config`, and only the Settings screen ever loaded `state.settings` — so `:setup` from Work mounted the wizard with no base document, every staging dispatch returned the state unchanged, and the done line still reported the repo as registered. The view effect now loads the config for the setup screen too, the screen holds the wizard behind a loading row until it arrives (which also fixes the once-only tool seed), the done line distinguishes staged from written, and `q` refuses while a document is staged — the last of which is why every screen renders `state.flash`, Work having been the only one that did. R11.8 and design_tui.md §14.2 amended in place (rev 31).
