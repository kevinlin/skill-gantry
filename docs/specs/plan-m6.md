...
- 2026-08-05 — **A polish pass over the screens this milestone shipped.** No new capability; four defects and a vocabulary that had drifted while six screens were built one at a time. The Work screen's half of the same pass, the output pane's scrolling, is in [plan-m2.md](plan-m2.md)'s changelog for this date.
  - **`q` in the in-session wizard quit the session.** Both key handlers are mounted while the setup screen is up, and this plan's own §14.2 precedence (mutation review, config confirm, setup, palette, help) was implemented everywhere except in `app.tsx`, where the `screen === 'setup'` guard sat *below* the bindings it was meant to outrank. So `q`, a character in half the repo paths a user types, reached the app's quit binding and took the staged configuration with it; `esc` jumped to Work instead of stepping back a state; `:` opened the palette over the wizard. `useSetupSession` had guarded its own keys against exactly this and said so in a comment. Guard hoisted, two regression tests added to `settings-edit.test.tsx`.
  - **The wizard's footer promised the wrong key.** Inside the session `q` returns to Settings; the footer said `quit`, which is what it does for `skillgantry setup` and not here. `Setup` takes an `exitLabel`, and its footer truncates rather than wraps — a longer label made the frame one row taller than a 50×14 terminal, which is §14.1's first rule broken by a caption.
  - **`low` severity was gray on Issues and the Dashboard and cyan in the findings pane.** One severity reading as two depending on the screen, and the cyan collided with the focus accent that marks the focused border, the selected tab and the cursor. Five colour maps in five files, already diverged, are now `src/tui/tokens.ts`: severity, outcome, job state and the accent, with the rule that only `running` may claim the accent. The same consolidation fixed a `skipped` stage painted the yellow of `errored` in the Dashboard's run history — a stage nobody ran reading as a stage that broke.
  - **Five phrasings of "you are seeing part of a list".** `+3 more`, `+3 more — keep typing`, `rows 1–12 of 40 · j/k scrolls`, `rows 1–12 of 40`, and nothing at all, so a user who learned one of them on the Dashboard learned it again on Issues. `overflowNotice()` states the position first, because `+3 more` says nothing about where in the list you are. It takes the recovery text from the caller: a pane the user cannot scroll advertising `j/k` is worse than saying nothing. The `+N more` riding `SkillList`'s and `Queue`'s titles stays: a hint beside a heading spends no row.
  - **Help hid its own keys.** Twenty-four flat rows, ordered by nothing, with six letters bound on more than one screen; at 80×24 the budget cut the last five, which were the global ones — including `q`. Regrouped by where a key works, global first, every screen-specific row naming its screen, rows that always travel together merged. Seventeen rows, which fits 80×24 whole.
  - **Copy.** `provenance(s)`, `path(s)` and `runtime(s)` against the proper pluralisation the rest of the app already used; the Issues footer omitted `q quit` while `q` quit from there, which made the key look screen-specific.
- 2026-08-05 — **The status bar names the running version.** A user reporting a screen's behaviour had nothing on screen to say which build they were looking at, and `skillgantry --version`, the other place to ask, answered `0.1.0` against a `package.json` reading `0.2.0`. The number was a literal in `run-command.ts` that nobody updated with the manifest.
