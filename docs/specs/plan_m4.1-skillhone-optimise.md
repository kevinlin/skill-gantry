...
- 2026-08-14 — **The optimise prompt names each finding's tool and how to accept it (R6.14).** Its per-stage findings table gained a tool column, and it composes the same accept block `fix-prompt.ts` does from `stages/prompt-parts.ts`, labelled by stage since it renders every stage of the run at once and each numbers from 1. Written up in [plan_m6.2](plan_m6.2-fix-prompts-for-stage-findings.md).
